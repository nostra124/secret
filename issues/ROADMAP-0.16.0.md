---
milestone: 0.16.0
theme: signing & identity — native crypto, Nostr/BIP-322 signing, secstored, mTLS peer sync
opened: 2026-07-01
status: planned
---
...
> BIP-322 sign-in, a local signer daemon (`secstored`), and mTLS peer
> sync. Spans 0.16.0 → 0.18.0. **Bugs come before features at the same
> priority.**

## The redesign, in one paragraph

The signer core lives in the engine library **`libsecstore`** (C, linking
**OpenSSL libcrypto** + **libsecp256k1**) so it is reachable three ways:
**inline** by the CLI,
**embedded** by the mobile apps (local stores, no daemon), and over an
**authenticated Unix socket** by `secstored`'s local clients. Those clients
link the thin **`libsecstore-client`** client library — no crypto, just the RPC
to `secstored` — for easy consumption and bindings. The only network
surface is **secstored ↔ secstored store sync over mTLS** — encrypted git
...
- **`libsecstore`** (engine + daemon) — store lifecycle, signer core,
  socket serving/auth, mTLS sync. Heavy deps. Linked by `secstored`, the
  CLI (inline), and mobile (embedded, in-process).
- **`libsecstore-client`** (client) — RPC to `secstored` over the Unix socket;
...
| Browser reach | **Local only** — WebExtension implements `window.nostr` (NIP-07) and bridges to `secstored` via a native-messaging host over the Unix socket. |
...
| ~~NIP-46 / remote bunker~~ | **Dropped** — remote signing is out of scope; there are no remote clients to `secstored`. |
...
Naming (decided): the engine keeps **`libsecstore`** and the
**`secstore_*`** symbol prefix — deliberately not `libsecret*`, preserving
the no-collision rationale with GNOME/freedesktop **libsecret**
(CLAUDE.md §4). The client library is **`libsecstore-client`**; the daemon
binary is **`secstored`**.

## 0.16.0 — crypto foundation & Nostr

- **FEAT-231** — add **`libsecstore-client`** (thin RPC client) alongside
  the unchanged **`libsecstore`** engine; introduce the **`secstored`**
  daemon binary. Completes alongside FEAT-228.
...
- **FEAT-228** — `secstored`: local signer daemon over an authenticated
...
- **FEAT-229** — mTLS peer sync **replacing** ssh/HTTP: secstored↔secstored
...
- **FEAT-230** — browser extension: `window.nostr` (NIP-07) via a
  native-messaging bridge to the `secstored` Unix socket.
