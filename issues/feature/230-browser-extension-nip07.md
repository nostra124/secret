...
2. A **native-messaging host** (`secret` invoked as the browser's native
   host) bridges the extension's stdio messages to the `secretd` Unix
   socket, presenting the extension's per-client token (FEAT-228).
3. Each signing request is subject to `secretd`'s approval model
...
2. Signing requests honour `secretd`'s approval policy and show the
   requesting origin.
